...
- **`node .factory/bin/qa-evidence.js`** — 증거를 남기는 **정본 도구**(ADR-024). 리다이렉션으로 직접 쓰지
  ...
  - `--` 뒤의 명령은 **당신이 직접 Bash로 쳤을 때와 똑같은 판정**을 받는다(도구가 스폰 전에 두 훅에
    ...
    `node -e …`, `python -c …`, 그리고 `env -S`·`xargs`·`timeout`·`nohup`으로 감싼 같은 것들):
    파이프라인이 필요하면 `/tmp`에 스크립트 파일을 만들어 `-- node /tmp/repro.js`처럼 **파일을 실행**한다.
    같은 이유로 **인자 안의 `>`·`;`도 훅에는 구분자로 보인다**(`-- jq '.a > 1' f.json`은 리다이렉션으로
    읽혀 거절된다) — 그런 인자가 필요하면 그것도 스크립트 파일로 옮긴다. 거절 문구가 당신이 쓰지도 않은
    리다이렉션을 말하면 그 이유다
  - `attach --file`의 출처는 저장소 안이나 `/tmp`여야 하고, 세션이 `Read(...)`로 막아 둔 파일
    (`.env*`·`.git/**`·`.npmrc`)은 증거가 될 수 없다 — 그 파일들은 애초에 이 세션이 읽지 않기로 한 것이다
