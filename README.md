...
서비스를 띄워 **implemented** 항목을 확인하려면(기본 3000 포트, 환경변수 PORT로 바꾼다):
...
이 README에서 백틱으로 적힌 저장소 경로는 전부 디스크에 실재해야 한다 — `test/readme.test.js`가
구분자도 확장자도 없는 한 단어 파일명까지 대조한다. 그래서 경로가 아닌 낱말(환경변수, 상태 어휘)은
백틱 대신 굵게 적는다.
