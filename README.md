...
2. DB 기동 — `--wait`은 Postgres의 healthcheck가 통과할 때까지 기다렸다가 반환한다.
   ...
   이 경고가 물리는 창은 **데이터 디렉터리를 처음 초기화하는 컨테이너**의 준비 시간이다.
   그 창 안에서 아래 `psql`을 부르면 소켓이 아직 없어 `No such file or directory`로 죽고,
   `--wait`을 붙이면 반환 직후의 같은 명령이 성공한다. 다만 `npm test`는 러너가 뜨는 1초
   남짓이 창을 가리기도 해서, 빠른 기계에서는 `--wait` 없이도 통과할 수 있다 — 이 실패는
   "항상"이 아니라 느린 기계나 이미지를 처음 내려받는 기계에서 **간헐적으로**, 그것도
   원인을 가리키지 않는 연결 오류로 나타난다. 이미 초기화를 마친 컨테이너를 다시 쓰면
   `--wait` 없이도 통과하므로, "지난번엔 없어도 됐다"도 이 줄을 지울 근거가 되지 못하고,
   이미 떠 있는 DB에 대고 한 번 더 돌린 실행은 이 순서를 검증하지 않는다.
...
1. 백틱으로 적은 저장소 경로는 디스크에 실재해야 한다. `docs/QA.md:37` 같은 줄 인용도 경로
   부분으로 판정하므로 인용을 백틱 밖으로 뺄 필요가 없다. 코드펜스 안의 명령도 같은 검사를
   받지만(예: `docker-compose.test.yml`), URL과 MIME 타입(**application/json**)과 프로토콜
   버전(**HTTP/1.1**)은 경로 주장이 아니다 — 헤더를 갖춘 정직한 curl 예시를 그대로 적어도 된다.
